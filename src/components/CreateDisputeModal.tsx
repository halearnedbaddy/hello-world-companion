...
export function CreateDisputeModal({ isOpen, onClose, onSuccess, userRole }: CreateDisputeModalProps) {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      // Get transactions based on user role - use lowercase status values
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Your dispute (Case #${disputeId.slice(0, 8)}) has been sent to our admin team for review. You'll receive updates via notifications.`,
      });
