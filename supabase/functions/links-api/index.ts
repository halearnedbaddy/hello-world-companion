...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (req.method === "POST" && pathAfter.length === 2 && pathAfter[1] === "purchase") {
      ...
      // Check quantity - only block if quantity tracking is enabled AND stock is depleted
      // quantity = null means unlimited (no tracking)
      // quantity = 0 when tracking is enabled means sold out
      // But if quantity was never set (default is 1 in DB), allow purchase
      const trackingQuantity = link.quantity !== null && link.quantity !== undefined;
      
      if (trackingQuantity && link.quantity <= 0) {
        // Check if this was intentionally sold out vs never tracked
        // If purchases > 0 and quantity is 0, it's sold out
        if (link.purchases && link.purchases > 0) {
          return new Response(
            JSON.stringify({ success: false, error: "This item is sold out", code: "OUT_OF_STOCK" }),
            { status: 400, headers: corsHeaders }
          );
        }
        // Otherwise, treat quantity 0 as "unlimited" for legacy/migration purposes
      }
      
      // If tracking quantity and requested quantity exceeds available
      if (trackingQuantity && link.quantity > 0 && link.quantity < quantity) {
        return new Response(
          JSON.stringify({ success: false, error: `Only ${link.quantity} items available`, code: "INSUFFICIENT_STOCK" }),
          { status: 400, headers: corsHeaders }
        );
      }
